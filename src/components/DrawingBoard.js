...
function setup(id, opts) {
	...
	this.dom.$customCursorStyleSheet = style.sheet
...
const CanvasMethods = {
    ...
    resize() {
        ...
        var widths = [
            ...
            DrawingBoard.Utils.boxBorderWidth(
                this.dom.$canvasWrapper,
                true,
                true
            ),
        ];
        var heights = [
            ...
            DrawingBoard.Utils.boxBorderHeight(
                this.dom.$canvasWrapper,
                true,
                true
            ),
        ...
        while ((pixel = queue.pop())) {
            ...
            if (
                ...
                if (pixel[X] > 0)
                    // west
                    queue.push(
                        DrawingBoard.Utils.pixelAt(img, pixel[X] - 1, pixel[Y])
                    );
                if (pixel[X] < maxX)
                    // east
                    queue.push(
                ...
                if (pixel[Y] > 0)
                    // north
                    queue.push(
                        DrawingBoard.Utils.pixelAt(img, pixel[X], pixel[Y] - 1)
                    );
                if (pixel[Y] < maxY)
                    // south
                    queue.push(
    ...
    initControls() {
        ...
        for (var i = 0; i < this.opts.controls.length; i++) {
            ...
            if (typeof this.opts.controls[i] == "string")
                c = new window["DrawingBoard"]["Control"][
                    this.opts.controls[i]
                ](this);
    ...
    addControl(control, optsOrPos, pos) {
        if (
            ...
            (typeof control !== "object" ||
                !control instanceof DrawingBoard.Control)
        )
            return false;
        ...
        if (typeof control == "string")
            control = new window["DrawingBoard"]["Control"][control](
                this,
                opts
            );
...
const WebStorageMethods = {
    ...
    _getStorage() {
        if (
            ...
            !(
                this.opts.webStorage === "session" ||
                this.opts.webStorage === "local"
            )
        )
            return false;
...
const IOMethods = {
    ...
    initKeyboardEvents() {
        const { redo, undo, clear, toggleOptions, colors, modes, size} = window.shortcuts;
		...
		Object.keys(modes).forEach(mode => {
			Mousetrap.bind(modes[mode], function(){
				this.setMode(mode)
			}.bind(that))
		})

		// Size
		Mousetrap.bind(size.increase, function(){
			this.ctx.lineWidth += 2
		}.bind(that))

		Mousetrap.bind(size.decrease, function(){
			this.ctx.lineWidth -= 2
		}.bind(that))
    ...
    initDrawEvents() {
        ...
        this.dom.$canvas.on("mousemove", function (e) {}.bind(this));
...
const DrawingModesMethods = {
	...
	cursorPen(fill) {
		...
		style="enable-background:new 0 0 32 32; fill: ${fill}"
		xml:space="preserve">
   		<g>
		...
		const rulesLength = this.dom.$customCursorStyleSheet?.cssRules?.length

		if (rulesLength > 0) {
			this.dom.$customCursorStyleSheet.removeRule(0);
		}
		
		this.dom.$customCursorStyleSheet.insertRule(svgRule, 0);
	},
